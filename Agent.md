...
3. 首次启动会出现一个「官方开源版」安全提示弹窗（确认一次后不再出现），属于同一防御机制的正常产品行为，不是 bug。
   **本 fork 已将弹窗内的开源地址改为 `https://github.com/xingmihai/YunX`**（原为上游 `CYQawa/YunX`）。
   弹窗文案存放在 `util/TextCipher.kt` 的 `D_BODY` / `D_URL`（XOR+Base64 密文），
   **改文案时必须重新生成密文，不得把明文写进源码**（会降低对抗静态分析的能力）。
   注意：真正的篡改检测逻辑在 `util/ArchiveProbe.kt`，与弹窗里的 URL 无关，换 URL 不影响检测能力。
