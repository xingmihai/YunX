...
interface Pan123AccountDao {
     ...
     */
    @Query("SELECT * FROM pan123_account WHERE isActive = 1 LIMIT 1")
    fun observeAccount(): Flow<Pan123AccountEntity?>

    /** 本平台全部已保存账号（最近更新的在前），用于账号切换列表 */
    @Query("SELECT * FROM pan123_account ORDER BY updatedAt DESC")
    fun observeAccounts(): Flow<List<Pan123AccountEntity>>
    ...
    @Query("UPDATE pan123_account SET isActive = 0")
    suspend fun clearActive()

    @Query("UPDATE pan123_account SET isActive = 1 WHERE id = :id")
    suspend fun markActive(id: String)

    /** 删除指定账号（其余账号不受影响） */
    @Query("DELETE FROM pan123_account WHERE id = :id")
    suspend fun deleteById(id: String)

    /** 清空本平台全部账号（退出登录沿用此行为，与单账号时代一致） */
    @Query("DELETE FROM pan123_account")
    suspend fun clear()
    /**
     * 按昵称查找已有账号的 id。
     *
     ...
     */
    @Query("SELECT id FROM pan123_account WHERE nickname = :nickname LIMIT 1")
     ...
     */
    suspend fun insertAsActive(account: Pan123AccountEntity) {
        clearActive()
    ...
    @Query("SELECT COUNT(*) FROM pan123_account WHERE isActive = 1")
    suspend fun countActive(): Int

    @Query("SELECT id FROM pan123_account ORDER BY updatedAt DESC LIMIT 1")
